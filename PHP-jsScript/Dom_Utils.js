...
function objWidth(objectID) {
    var obj = xDOM(objectID,0); if(obj.offsetWidth)
...
function loadImage(im){
         var i=new Image();
         i.src='PHP-images/SubButt'+im+'.jpg';
         return i;
}
...
function getBtnVal () {
    ...
    if ( obj.value == "" ) obj.value="Homeselect";
    btnSelect(obj.value);
//    cb = obj.value;
}
function msOver(id){
  x=id.substring(0,id.length-4);
  y=cb.substring(0,cb.length-6);
  if ( x != y )
...
function msOut(id){
  x=id.substring(0,id.length-2);
  y=cb.substring(0,cb.length-6);
  yy=cb.substring(cb.length-6);
  if ( x != y  )
    document['btn'+x].src=eval('btn'+id+'.src');
    if(id.indexOf('e')!=-1)
        document['btn'+x+'e'].src=eval('btn'+id+'e.src');
