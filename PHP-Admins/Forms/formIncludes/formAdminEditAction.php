...
switch ($confModel->adminhtmltag){
    ...
    case "textarea":
		...
		echo "<td valign='top'>".$limg."<textarea id='".$key."' name='".$key."' ".$confModel->adminhtmltype." >".$val."</textarea>".$img."</td>";
        break;
    case "list":
        if ( $fileKey == $key ) {
            $hdLabel = getLabel("current",$locale);
    ...
    case "select":
        ...
        switch ($confModel->selvaltype){
			...
			echo "<td  valign='top'><select id='".$key.$multi."' name='".$key.$multi."' ".$confModel->adminhtmltype." onchange=\"javascript:".$confModel->jscript.";\" >";
            if ( $key == 'emailid' )
    ...
    Default:
        ...
        switch ($confModel->adminhtmltype) {
            ...
            case "checkbox":
                ...
                if ( $confModel->conf_value == "" || strpos($confModel->conf_value,",") === false ) {
					...
					echo "<td valign='top'><input type='checkbox' name=checkbox_".$val." id=checkbox_".$val." value='".$val."' ".$checked."/></td>";
                } else {
		...
		Default:
			...
			echo "<td nowrap valign='top'>".$hdrLabel."</td>";
			echo "<td valign='top'>$limg<input type=".$confModel->adminhtmltype." id='".$key."' name='".$key."' value='$v'/>".$img."</td>";
            }
        }
        if ( $enc_password && $key == $enc_key ) {
